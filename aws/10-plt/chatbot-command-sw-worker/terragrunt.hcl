# -----------------------------------------------------------------------------
# Chatbot Short-Write (SW) Unified Worker - Platform
# cloud-sandbox/aws/10-plt/chatbot-command-sw-worker/terragrunt.hcl
#
# Unified Lambda worker for short-lived write commands
# Handles: /scale, /restart, and future short-write commands
# -----------------------------------------------------------------------------
...
inputs = {
  ...
  description   = "Unified worker for all short-write commands (scale, restart, etc.)"

  runtime = "nodejs20.x"
  handler = "workers/sw/index.handler"
  ...
  tags = merge(
    include.env.locals.common_tags,
    {
      ...
      Component       = "command-sw-worker"
      Quadrant        = "sw"
      QuadrantName    = "short-write"
      CommandCategory = "short-write"
      SLOTarget       = "p99-2s"
    }
  )
}
